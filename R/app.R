...
server <- function(input, output) {
  ...
  observeEvent(input$FinalCheck, {
    if (length(parseFilePaths(roots, input$files)$datapath)!=0) {
      ...
      ColOne <- as.data.frame(formatC(as.numeric(ColOne[,1]), format = 'e', flag='0', digits = 7))
      print("One Done")
      print(ColOne)
      #ColTwo <- as.data.frame(lapply(ColTwo, sprintf, fmt = "%e"))
      ColTwo <- as.data.frame(formatC(as.numeric(ColTwo[,1]), format = 'e', flag='0', digits = 7))
      print("Two Done")
      print(ColTwo)
      #ColThree <- as.data.frame(lapply(ColThree, sprintf, fmt = "%e"))
      ColThree <- as.data.frame(formatC(as.numeric(ColThree[,1]), format = 'e', flag='0', digits = 7))
      print("Three Done")
      print(ColThree)
      #ColFour <- as.data.frame(lapply(ColFour, sprintf, fmt = "%e"))
      ColFour <- as.data.frame(formatC(as.numeric(ColFour[,1]), format = 'e', flag='0', digits = 7))
      print("Four Done")
      print(ColFour)
